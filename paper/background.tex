...
\citep{latif_agi_2024} - AGI systems can create personalized learning tracks for students by learning how the students learn and then adjusting how to teach. It can also help developing nations get high level education. By allowing fine-tuning, the schools can make the content specific for what they want to teach when they might be out of distribution.
\citep{european_commission_impact_2022} - AI will probably take jobs that include pattern recognition, judgement, and optimization. Previous technological innovations has augmented what humans have done for jobs.
\citep{gruetzemacher_transformative_2021} - AI could be a general purpose technology that has the potential to change society fundamentally at the level of mass production or electricity. Leading to us being reliant on it fundamentally.


Open 7 - Open-source AI reduces the cost of accessing advanced technology for startups, researchers, and developing nations.
\citep{jones_nonrivalry_2020} - Data is nonrival, so by hoarding data or governments restricting data sharing, valuable economic growth is lost.
...
\citep{touvron_llama_2023} - Training on public data can get the same results as proprietary data. They also did not release the training code, just the models.
\citep{eiras_near_2024} - By releasing the models to the public, the economical gains can be achieved, but hiding some methods to get their may be necessary to keep insensitive for investments into large and more innovative models.

No 10 - Strict content filtering makes AI less useful and amounts to unnecessary censorship.
\citep{perrigo_elon_2023} - Elon Musk has argued that by censoring the models output may make then less truthful, thus reducing their economic benefits.

Pro 9 - AI systems used in important decisions should always explain their reasoning in ways humans can understand.
\citep{nelson_elhage_mathematical_2021} - Efforts to understand circuits within attention heads. 
\citep{meng_locating_2023} - Facts are stored in the MLP right after the attention in the blocks of a transformer. They were able to update these memory banks to change facts within the model. They note that there are serious ethical concerns over this as misinformation could be put into the models.
...
\citep{whitehouse_removing_2025} - The us government has made it clear that it want to be a clear leader in the space of AI innovation.
